...
void	draw_stack(t_mlx *m, t_stack *stack, int x, int height)
{
	...
	put_rectangle(m, new_point(x, 0, stack->head->color),
	((stack->head->index + 1) * m->width / 2) / (m->max_index + 1), height);
	head = stack->head->next;
	y = height;
	while (head != stack->head)
	{
		put_rectangle(m, new_point(x, y, head->color), ((head->index + 1)
		* m->width / 2) / (m->max_index + 1), height);
		y += height;
		head = head->next;
	}
}
